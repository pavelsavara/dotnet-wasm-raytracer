...
// code downloaded from http://www.nokola.com/Raytracer/HowToBuild.aspx
...
public class RayTracer
{
    ...
    private Color GetNaturalColorInternal(SceneObject thing, Vector pos, Vector norm, Vector rd, Scene scene)
    {
        ...
        foreach (Light light in scene.Lights)
        {
            ...
            if (!isInShadow)
            {
                ...
                Color lColorInternal = illum > 0 ? Color.Times(illum, light.ColorInternal) : Color.Make(0, 0, 0);
                double specular = Vector.Dot(livec, Vector.Norm(rd));
                Color sColorInternal = specular > 0 ? Color.Times(Math.Pow(specular, thing.Surface.Roughness), light.ColorInternal) : Color.Make(0, 0, 0);
                ret = Color.Plus(ret, Color.Plus(Color.Times(thing.Surface.Diffuse(pos), lColorInternal),
                                                 Color.Times(thing.Surface.Specular(pos), sColorInternal)));
            }
        }
        return ret;
    }

    private Color GetReflectionColorInternal(SceneObject thing, Vector pos, Vector norm, Vector rd, Scene scene, int depth)
    {
    ...
    private Color Shade(ISect isect, Scene scene, int depth)
    {
        ...
        Color ret = Color.DefaultColorInternal;
        ret = Color.Plus(ret, GetNaturalColorInternal(isect.Thing, pos, normal, reflectDir, scene));
        if (depth >= MaxDepth)
        {
            return Color.Plus(ret, Color.Make(.5, .5, .5));
        }
        return Color.Plus(ret, GetReflectionColorInternal(isect.Thing, Vector.Plus(pos, Vector.Times(.001, reflectDir)), normal, reflectDir, scene, depth));
    }
    ...
    {
        byte[] canvas = new byte[screenHeight * screenWidth * 4];
        for (int y = 0; y < screenHeight; y++)
        {
            for (int x = 0; x < screenWidth; x++)
            {
                Color ColorInternal = TraceRay(new Ray() { Start = scene.Camera.Pos, Dir = GetPoint(x, y, scene.Camera) }, scene, 0);
                var red = 4 * (screenHeight * y + x);
                canvas[red] = (byte)(ColorInternal.R * 256);
                canvas[red + 1] = (byte)(ColorInternal.G * 256);
                canvas[red + 2] = (byte)(ColorInternal.B * 256);
                canvas[red + 3] = 255;
            }
        }
        return canvas;
        ...
        new Scene()
        {
            ...
            Lights = new Light[] {
                                new Light() {
                                    ...
                                    ColorInternal = Color.Make(.49,.07,.07)
                                },
                                new Light() {
                                    Pos = Vector.Make(1.5,2.5,1.5),
                                    ColorInternal = Color.Make(.07,.07,.49)
                                },
                                new Light() {
                                    Pos = Vector.Make(1.5,2.5,-1.5),
                                    ColorInternal = Color.Make(.07,.49,.071)
                                },
                                new Light() {
                                    Pos = Vector.Make(0,3.5,0),
                                    ColorInternal = Color.Make(.21,.21,.35)
                                }},
...
public class Color
{
    ...
    public static readonly Color DefaultColorInternal = Make(0, 0, 0);
